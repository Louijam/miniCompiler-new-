...
inline ast::Type value_to_type(const Value& v) {
    ...
    if (auto* o = std::get_if<ObjectPtr>(&v)) return ast::Type::Class((*o)->class_name);
...
inline LValue eval_lvalue(Env& env, const ast::Expr& e, FunctionTable& functions) {
    ...
    if (auto* m = dynamic_cast<const MemberAccessExpr*>(&e)) {
        Value ov = eval_expr(env, *m->object, functions);
        if (!std::holds_alternative<ObjectPtr>(ov)) throw std::runtime_error("runtime error: member access on non-object");
        ObjectPtr obj = std::get<ObjectPtr>(ov);
        return LValue::field_ref(obj, m->field);
    }
